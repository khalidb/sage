...
public class MatrixEvidenceVector {
	
	public RealMatrix matrix;

	
	public MatrixEvidenceVector(int[][] value){
        
		double[][] doubleData = new double[value.length][value[0].length];
		
		for (int i = 0; i < value.length; i++) {
            for (int j = 0; j < value[0].length; j++) {
                doubleData[i][j] = value[i][j];
            }
        }
		matrix = new Array2DRowRealMatrix(doubleData);
	}
	
	public MatrixEvidenceVector(double[][] value){
        
		matrix = new Array2DRowRealMatrix(value);
	}
	...
	public static void print(MatrixEvidenceVector rev, int num_attrs, List<OpenBitSet> possible_bitsets) {
